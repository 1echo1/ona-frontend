...
type LoginResult =
  ...
  | { success: false; errorCode: 'invalid_credentials' | 'network_error' | 'unknown' };

export default async function LoginRequest(email: string, password: string): Promise<LoginResult> {
  try {
